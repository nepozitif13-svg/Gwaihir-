...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Проверяем инвайт-код на сервере
    ...
    const { inviteId } = await check.json();

    // Регистрируем пользователя
    const { data, error: signUpError } = await supabase.auth.signUp({
      email,
      password,
      options: { data: { invite_id: inviteId } },
    });
  ...
  return (
    <main className="min-h-screen bg-zinc-950 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <p className="text-xs text-zinc-500 mb-6">Invite code required.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full bg-zinc-900 border border-zinc-700 rounded px-3 py-2 text-sm text-zinc-100 focus:outline-none focus:border-zinc-500 font-mono"
        ...
        <p className="text-xs text-zinc-500 mt-4 text-center">
          ...
          <a href="/login" className="text-zinc-400 hover:text-zinc-200">Sign in</a>
        </p>
      </div>
    </main>
  );
